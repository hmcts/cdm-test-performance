...
class CCD_PerformanceRegression extends Simulation  {
  ...
  val etCaseworkerTargetPerHour:Double = 100 //30
  val etCitizenTargetPerHour:Double = 100 //30
  val caseFileViewTargetPerHour:Double = 100 //30
...
//  val config: Config = ConfigFactory.load()
  ...
  val API_STCreateCase = scenario("Special Tribs Case Create")
    .exitBlockOnFail {
      exec(_.set("env", s"${env}"))
        .exec(S2S.s2s("ccd_data"))
        .feed(feedSTUserData)
        .exec(IdamLogin.GetIdamToken)
        .exec(ccddatastore.CCDAPI_SpecialTribunalsCreate)
  ...
  val API_ET1SolicitorCreateProgressAndET3Response = scenario("ET1 Solicitor Create and Progress Case then ET3 Response")
    .exitBlockOnFail {
      ...
      .exec(ccddatastore.IDAM_DeleteCitizenRespondentUser)
    }

  val API_ET1CitizenCreate = scenario("ET1 Citizen Create Case")
    .exitBlockOnFail {
      exec(_.set("env", s"${env}"))
      .exec(_.set("Jurisdiction", "EMPLOYMENT"))
      .exec(_.set("CaseType", "ET_EnglandWales"))
      .exec(S2S.s2s("ccd_data"))
      //.feed(feedETCitizenData)
      .exec(ccddatastore.IDAM_CreateCitizenAppUser)
      .exec(IdamLogin.GetIdamToken)
      ...
      .exec(ccddatastore.IDAM_DeleteCitizenUser)
    }

  val API_ETViewCaseAndEvents = scenario("ET View Case and Get Events")
    .exitBlockOnFail {
      exec(_.set("env", s"${env}"))
      .exec(S2S.s2s("ccd_data"))
      .feed(feedETCaseData)
      .feed(feedETUserData)
      .exec(IdamLogin.GetIdamToken)
      .exec(ccddatastore.CCDAPI_ETViewCase)
      .exec(ccddatastore.CCDAPI_ETGetEvents)
    }
